...
class RobotSprite:
    ...
    def transform(pose, polygon):
        ...
        for point in polygon:
            x = (
                pose.position.x
                + point[0] * cos(pose.rotation)
                - point[1] * sin(pose.rotation)
            )
            y = (
                pose.position.y
                + point[0] * sin(pose.rotation)
                + point[1] * cos(pose.rotation)
            )
    ...
    def draw(self, window, pose):
        ...
        DrawingUtils.draw_polygon_on_screen(
            window, RobotSprite.transform(pose, self.left_wheel), self.wheel_color, 0
        )
        DrawingUtils.draw_polygon_on_screen(
            window, RobotSprite.transform(pose, self.right_wheel), self.wheel_color, 0
        )
        DrawingUtils.draw_polygon_on_screen(
            window, RobotSprite.transform(pose, self.body), self.robot_color, 0
        )
        DrawingUtils.draw_polygon_on_screen(
            window, RobotSprite.transform(pose, self.line_sensor), self.robot_color, 0
        )
...
class Simulation:
    ...
    def update_line_sensor_intensity(self):
        ...
        for i in range(len(sensor_positions)):
            for piece in self.track.pieces:
                ...
                intensity[i] = max(
                    intensity[i], max(0.0, (sensor_range - distance) / sensor_range)
                )
    ...
    def update(self):
        ...
        self.point_list.append(
            (
                m2pix(self.line_follower.pose.position.x),
                m2pix(self.line_follower.pose.position.y),
            )
        )
    ...
    def evaluate(self):
        ...
        robot_direction = Vector2(
            cos(self.line_follower.pose.rotation), sin(self.line_follower.pose.rotation)
        )  # r_k
        dot_product = track_tangent.dot(robot_direction)  # dot(r_k, t_k)
        # Todo: implement

        error = 3 if not detection else error
        return dot_product * linear - 9 * abs(error)
